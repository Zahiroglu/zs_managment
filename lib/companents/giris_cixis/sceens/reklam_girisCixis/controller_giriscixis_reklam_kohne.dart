...
class ControllerGirisCixisReklamKohne extends GetxController {
  ...
  Future<void> getAllDataFormLocale() async {
    ...
    for (MercDataModel model in listmodel) {
      ...
      for (MercCustomersDatail modelMerc in musteriler) {
        ...
        modelCari.rutGunu = rutGunu ? "right".tr : "wrong".tr;
  ...
  ///cixis ucun hazirliq
  pripareForExit(Position currentLocation, String uzaqliq,
  ...
  Future<void> girisiLocaldaTesdiqleme(Position currentLocation,
    ...
    checkUnsededAllVisitsInEnter(
        currentLocation, selectedModel, uzaqliq, modela);
    update();
  }

  Future<void> checkUnsededAllVisitsInEnter(
      Position currentLocation,
      ModelCariler selectedModel,
      String uzaqliq,
      ModelCustuomerVisit modela) async {
    ...
    if (countUnsended > 0) {
      ...
      _callsendAllUnsendedVisitsPoinsEnter(
          model, currentLocation, selectedModel, uzaqliq, modela);
    } else {
      await girisiTamTesdiqle(currentLocation, uzaqliq, modela, selectedModel);
    }
  }

  ///rest ip gonderilmemis girisler ucun
  Future<void> _callsendAllUnsendedVisitsPoinsEnter(
      ModelCustuomerVisit modelvisit,
      Position currentLocation,
      ModelCariler selectedModel,
      String uzaqliq,
      ModelCustuomerVisit mainModel) async {
    await userService.init();
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (countUnsended > 0) {
          ModelCustuomerVisit model =
              localDbGirisCixis.getAllUnSendedGirisCixis().first;
          _callsendAllUnsendedVisitsPoinsEnter(
              model, currentLocation, selectedModel, uzaqliq, mainModel);
        } else {
          checkUnsededAllVisitsInEnter(
              currentLocation, selectedModel, uzaqliq, mainModel);
        }
      } else {
        if (ModelExceptions.fromJson(response.data['exception']).code != null) {
          print("_callsendAllUnsendedVisitsPoinsEnter --Start");
          if (ModelExceptions.fromJson(response.data['exception']).code ==
              "010") {
            print("_callsendAllUnsendedVisitsPoinsEnter --Start-code = 010");

            Get.dialog(ShowInfoDialog(
                messaje: ModelExceptions.fromJson(response.data['exception'])
                    .message!,
                icon: Icons.error,
                callback: () {
                  DialogHelper.hideLoading();
          ...
          DialogHelper.hideLoading();
          await girisiTamTesdiqle(
              currentLocation, uzaqliq, mainModel, selectedModel);
        }
      }
      ...
      await girisiTamTesdiqle(
          currentLocation, uzaqliq, mainModel, selectedModel);
      print("Error messaje :" + e.message!);
    }
  }

  Future<void> girisiTamTesdiqle(Position? currentLocation, String uzaqliq,
      ModelCustuomerVisit? modela, ModelCariler? selectedModel) async {
    await _callApiForEnter(currentLocation!, selectedModel!, modela!)
        .whenComplete(() async {
  ...
  Future<void> _callApiForEnter(Position currentLocation,
      ModelCariler selectedModel, ModelCustuomerVisit modela) async {
    print("Giris ucun Gonderilme endpointy cagrildi.Gonderilecek deyer sayi");
    await userService.init();
    LoggedUserModel loggedUserModel = userService.getLoggedUser();
    ModelRequestGirisCixis model = ModelRequestGirisCixis(
        ...
        operationType: "In",
        operationDate: DateTime.now().toString().substring(0, 18),
        userCode: loggedUserModel.userModel!.code!.toString());
    print("Giris model :" + model.toString());
    DialogHelper.hideLoading();
    ...
    if (connectivityResult == ConnectivityResult.none) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          modela.gonderilme = "1";
          localDbGirisCixis.updateSelectedValue(modela);
          funFlutterToast("ugurluApiGiris", true);
          DialogHelper.hideLoading();
        } else {
          if (ModelExceptions.fromJson(response.data['exception']).code !=
              null) {
            print("_callApiForEnter --Start");
            if (ModelExceptions.fromJson(response.data['exception']).code ==
                "010") {
              print("_callApiForEnter --Start code=010");
            ...
            funFlutterToast("errorApiGiris", false);
            DialogHelper.hideLoading();
          }
  ...
  ///Cixis ucun emeliyyatlar

  Future<void> cixisiLocaldaTesdiqle(
      Position currentLocation, String uzaqliq, String qeyd) async {
    circles.clear();
    ModelCustuomerVisit modela = ModelCustuomerVisit(
        operationType: "out",
        ...
        inDate: modelgirisEdilmis.value.inDate!.add(const Duration(seconds: 2)),
        gonderilme: "0",
        outNote: qeyd,
    ...
    checkUnsededAllVisitsInExit(currentLocation, uzaqliq, modela);
    update();
  }

  Future<void> checkUnsededAllVisitsInExit(Position currentLocation,
      String uzaqliq, ModelCustuomerVisit modela) async {
    int countUnsended = localDbGirisCixis.getAllUnSendedGirisCixis().length;
    if (countUnsended > 0) {
      ModelCustuomerVisit model =
          localDbGirisCixis.getAllUnSendedGirisCixis().first;
      await _callsendAllUnsendedVisitsPoinsExit(
          model, currentLocation, uzaqliq, modela);
    } else {
      await cixisiTamTesdiqle(
          currentLocation, uzaqliq, ctCixisQeyd.text, modela);
    }
  }

  Future<void> _callsendAllUnsendedVisitsPoinsExit(
      ModelCustuomerVisit modelvisit,
      Position currentLocation,
      String uzaqliq,
      ModelCustuomerVisit mainModel) async {
      ...
      if (response.statusCode == 200) {
        ...
        if (countUnsended > 0) {
          ModelCustuomerVisit model =
              localDbGirisCixis.getAllUnSendedGirisCixis().first;
          await _callsendAllUnsendedVisitsPoinsExit(
              model, currentLocation, uzaqliq, mainModel);
        } else {
          checkUnsededAllVisitsInExit(currentLocation, uzaqliq, mainModel);
        }
      } else {
        if (ModelExceptions.fromJson(response.data['exception']).code != null) {
          print("_callsendAllUnsendedVisitsPoinsExit --Start");
          if (ModelExceptions.fromJson(response.data['exception']).code ==
              "010") {
            print("_callsendAllUnsendedVisitsPoinsExit --Start code =010");
            DialogHelper.hideLoading();
            funFlutterToast("Melumat gonderilmedi xetaaaa", false);
            Get.dialog(ShowInfoDialog(
            ...
            DialogHelper.hideLoading();
            cixisiTamTesdiqle(
                currentLocation, uzaqliq, ctCixisQeyd.text, mainModel);
          }
        } else {
          DialogHelper.hideLoading();
        }
  ...
  Future<void> cixisiTamTesdiqle(Position? currentLocation, String uzaqliq,
      String qeyd, ModelCustuomerVisit? modela) async {
    await _callApiForExit(currentLocation!, uzaqliq, ctCixisQeyd.text, modela!)
        .whenComplete(() async {
      if (listCariler
          .any((a) => a.code == modelgirisEdilmis.value.customerCode)) {
        ModelCariler modelCari = listCariler
            .where((a) => a.code == modelgirisEdilmis.value.customerCode)
            .first;
        modelCari.ziyaret = "2";
      ...
      localDbGirisCixis.addSelectedGirisCixisDB(modela);
      marketeGirisEdilib.value = false;
      slidePanelVisible.value = false;
      modelgirisEdilmis.value = ModelCustuomerVisit();
      _timer!.cancel();
  ...
  Future<void> _callApiForExit(Position currentLocation, String uzaqliq,
    ...
    print("Cixis ucun Gonderilme endpointy cagrildi.Gonderilecek deyer sayi");
  }
  ...
  Future<void> checkAllVisits(Position currentLocation,
    ...
    int countUnsended = localDbGirisCixis.getAllUnSendedGirisCixis().length;
    if (countUnsended > 0) {
      ModelCustuomerVisit model = localDbGirisCixis.getAllUnSendedGirisCixis().first;
      await _callApiForSendUnsededs(model, currentLocation, uzaqliq, selectedModel, isEnter, modela);
    } else {
      if (isEnter) {
        _callApiForVisits(currentLocation, uzaqliq, selectedModel, isEnter, modela);
      } else {
        _callApiForVisits(currentLocation, uzaqliq, selectedModel, isEnter, modela);
      }
    }
  ...
  Future<void> _callApiForSendUnsededs(ModelCustuomerVisit modelvisit, Position currentLocation,
      String uzaqliq, ModelCariler selectedModel, bool isEnter, ModelCustuomerVisit mainVisitModel) async {
    DialogHelper.showLoading("Kohne melumatlar gonderilir");
    await userService.init();
    LoggedUserModel loggedUserModel = userService.getLoggedUser();
    ModelRequestGirisCixis model = ModelRequestGirisCixis();
      ...
      if (response.statusCode == 200) {
        ...
        if (ModelExceptions.fromJson(response.data['exception']).code != null) {
          ...
          if (expCode == "010") {
            ...
            DialogHelper.hideLoading();
            Get.dialog(ShowInfoDialog(
                messaje: "$messsaje.Sistemi yeniden baslatmaq lazimdir",
                icon: Icons.error,
                callback: () {
                  DialogHelper.hideLoading();
  ...
  Future<void> _callApiForVisits(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        DialogHelper.hideLoading();
        modelVisit.gonderilme = "1";
        localDbGirisCixis.addSelectedGirisCixisDB(modelVisit);
        checkAllVisits(currentLocation, selectedModel, uzaqliq, isEnter);
      } else {
        if (ModelExceptions.fromJson(response.data['exception']).code != null) {
          String expCode = ModelExceptions.fromJson(response.data['exception']).code.toString();
  ...
  Future<void> cixisiLocaldaTesdiqleLast(Position? currentLocation,
      String uzaqliq, String qeyd, ModelCustuomerVisit? modela) async {
    if (listCariler
        .any((a) => a.code == modelgirisEdilmis.value.customerCode)) {
      ModelCariler modelCari = listCariler
          .where((a) => a.code == modelgirisEdilmis.value.customerCode)
